...
学习模块：负载问题的pattern，根据论文中的经验，故障的根因一般源于底层物理机层面的问题。在实现负载均衡的背景下，就要应让模型知道什么样的 指标pattern 代表即将到来的负载问题。根因（表现为 metric 异常）一般会在表象（KPI）异常提前4h出现，因此在这段时间内的准确预测和识别，并触发动态负载均衡机制（器）可以有效缓解故障出现。

监测模块；metric异常阈值触发预测器，SPOT

预测模块：面向类型聚类、决策树，通过该结果判断负载机状态，就可以二分（正常、过载），即将过载就触发治理模块
