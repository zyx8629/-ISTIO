...
2⃣️ 故障检测——SPOT算法 + TCORW算法 + 排序算法

度量值异常的发现：SPOT通过极值理论来检测时间序列的突变。异常检测模块完成后，输出各指标的异常时间和异常程度。

故障的诊断：在故障因果图上进行基于时间的随机游走模型——>转移概率矩阵，其中利用person相关系数计算两种指标的相关性——>访问最多的可能是根因，那就是转移概率最大的？？

潜在根因的打分：指标被访问次数与异常程度的拟合的综合考量。

根因排序：

输入：优先级（专家划归的）、根因打分、异常时间
...
V0: 多维度监测数据，发现问题，对实现LB起作用，找自己的规则，使用数理统计或深度学习的方法。

V1: 【特征工程】特征提取、融合（特征来自多维metric，同时感觉一定要考虑时间序列特性）-> 【问题识别】基于机器学习特征，识别、自动发现负载不均衡点（这里可以考虑将问题分类) -> 【治理优化】治理不均衡（不同类型的问题采取不同的治理方法：熔断、流量转发、扩容、调度）

V2: 

题目：容器化服务平台中动态智能负载均衡机制的研究

模型整体可分为两个阶段：离线学习 和 在线预测（识别），四个模块：学习模块、监测模块、预测模块、治理模块

学习模块：负载问题的pattern，根据论文中的经验，故障的根因一般源于底层物理机层面的问题。在实现负载均衡的背景下，就要应让模型知道什么样的 指标pattern 代表即将到来的负载问题。由于微服务间存在故障传播的延时性，根因（metric 异常）一般会比表象（KPI）异常提前 4h 出现。因此，该模块需要学习识别历史数据中，KPI异常时刻的前四个小时的metric pattern。后期预测模型触发时，可准确识别和预测，并触发动态负载均衡机制（器）可以有效缓解故障出现。

感觉在特定领域问题下可只考虑相关mertic，web QPS 和 中间件QPS

将 metric 的异常 pattern 表现为四种异常类型：峰值、水平上移、水平下移（出现长时间的突然上升/下降或上升/下降）和 无效（零或缺失）

监测模块；metric异常阈值触发预测器，SPOT（可输出异常强度）

预测模块：基于机器学习的异常 pattern 提取算法，异常强度等级，通过该等级结果评估负载机状态，就能得到二进制结果就行（正常、异常），即将异常（过载就触发治理模块

治理模块：弹性扩容（HPA），水平增加负载容器，等指标正常后再缩减，减少资源消耗。或者保留新增容器，但是流量重新引回，以便下次分流负载准备。

v3:

题目：（科技服务协同平台）容器化服务平台动态优化技术的研究

模型整体可分为两个阶段：离线学习 和 在线监测、治理，四个模块：分析模块、学习模块、监测模块、治理模块

第一个模块基于统计学原理进行度量异常模型的特征提取工作；
第二个模块进行基于机器学习的度量异常模型的特征学习工作，针对历史度量数据中实际的故障模型进行学习，训进该框架可以实现精准识别异常的功能；
第三个模块属于在线阶段，主要实现实时且连续的监测功能，该模块需对平台运行时度量数据进行实时监测，并能根据上一阶段中学习的度量异常模型持续发现异常的到来，并判断异常程度是否触发下一模块中的负载均衡器；
第四个模块中配置了一种有效的弹性负载均衡器，主要基于上一模块提供的平台运行时监测的度量数据进行异常的自动化感知并利用容器的易伸缩、易扩展的性质实现缓解潜在风险的目标。
